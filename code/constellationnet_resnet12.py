...
class ConstellationNet_resnet12(torch.nn.Module):
    
    def __init__(self, nb_clusters, nb_heads, nb_epochs, beta, lbda, n_channels_start, output_size):
        ...
        self.concatPart_1 = torch.nn.Sequential(
            torch.nn.Conv2d(nb_clusters+64, n_channels_start, 1),
            torch.nn.BatchNorm2d(n_channels_start),
            torch.nn.ReLU()
        )
        
        self.concatPart_2 = torch.nn.Sequential(
            torch.nn.Conv2d(nb_clusters+128, n_channels_start, 1),
            torch.nn.BatchNorm2d(n_channels_start),
            torch.nn.ReLU()
        )

        self.concatPart_3 = torch.nn.Sequential(
            torch.nn.Conv2d(nb_clusters+256, n_channels_start, 1),
            torch.nn.BatchNorm2d(n_channels_start),
            torch.nn.ReLU()
        )

        self.concatPart_4 = torch.nn.Sequential(
            torch.nn.Conv2d(nb_clusters+512, n_channels_start, 1),
            torch.nn.BatchNorm2d(n_channels_start),
            torch.nn.ReLU()
        )

        self.resnet12_b1 = torch.nn.Sequential(
          torch.nn.Conv2d(n_channels_start, 64, 3, padding = 1),
          torch.nn.BatchNorm2d(64),
          torch.nn.LeakyReLU(0.01),
          torch.nn.MaxPool2d(2)
        )

        self.resnet12_b2 = torch.nn.Sequential(
          torch.nn.Conv2d(n_channels_start, 128, 3, padding = 1),
          torch.nn.BatchNorm2d(128),
          torch.nn.LeakyReLU(0.01),
          torch.nn.MaxPool2d(2)
        )

        self.resnet12_b3 = torch.nn.Sequential(
          torch.nn.Conv2d(n_channels_start, 256, 3, padding = 1),
          torch.nn.BatchNorm2d(256),
          torch.nn.LeakyReLU(0.01),
          torch.nn.MaxPool2d(2)
        )

        self.resnet12_b4 = torch.nn.Sequential(
          torch.nn.Conv2d(n_channels_start, 512, 3, padding = 1),
    ...
    def forward(self, x):
        for i in tqdm(range(4)):
            ...
            if i == 0:
              cfm = self.resnet12_b1(x)
            elif i == 1:
              cfm = self.resnet12_b2(x)
            elif i == 2:
              cfm = self.resnet12_b3(x)
            elif i == 3:
              cfm = self.resnet12_b4(x)
            else:
              return None
            ...
            if i == 0:
              x = self.concatPart_1(x_concat)
            elif i == 1:
              x = self.concatPart_2(x_concat)
            elif i == 2:
              x = self.concatPart_3(x_concat)
            elif i == 3:
              x = self.concatPart_4(x_concat)
            else:
              return None
